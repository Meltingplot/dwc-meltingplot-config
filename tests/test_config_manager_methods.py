...
@pytest.fixture
def mock_dsf():
    """Create a mock DSF connection."""
    dsf = MagicMock()
    dsf.get_file = MagicMock(return_value=None)
    dsf.put_file = MagicMock()
    return dsf


@pytest.fixture
def manager(mock_dsf):
    """Create a ConfigManager with mocked DSF and init_backup_repo."""
    with patch("config_manager.init_backup_repo"):
        mgr = ConfigManager(dsf_command_connection=mock_dsf)
    return mgr
...
class TestFileIO:
    def test_read_printer_file_success(self, manager, mock_dsf):
        mock_dsf.get_file.return_value = "G28\n"
        result = manager._read_printer_file("0:/sys/config.g")
        assert result == "G28\n"
        mock_dsf.get_file.assert_called_once_with("0:/sys/config.g")

    def test_read_printer_file_exception_returns_none(self, manager, mock_dsf):
        mock_dsf.get_file.side_effect = IOError("file not found")
        result = manager._read_printer_file("0:/sys/config.g")
        assert result is None

    def test_read_printer_file_no_dsf_raises(self):
        with patch("config_manager.init_backup_repo"):
            mgr = ConfigManager(dsf_command_connection=None)
        with pytest.raises(RuntimeError, match="No DSF connection"):
            mgr._read_printer_file("0:/sys/config.g")

    def test_write_printer_file_success(self, manager, mock_dsf):
        manager._write_printer_file("0:/sys/config.g", "G28\n")
        mock_dsf.put_file.assert_called_once_with("0:/sys/config.g", "G28\n")

    def test_write_printer_file_no_dsf_raises(self):
        with patch("config_manager.init_backup_repo"):
            mgr = ConfigManager(dsf_command_connection=None)
        with pytest.raises(RuntimeError, match="No DSF connection"):
...
class TestDiffAll:
    ...
    def test_diff_all_unchanged_file(self, manager, mock_dsf, tmp_path):
        git_dir = tmp_path / ".git"
        git_dir.mkdir()
    ...
    def test_diff_all_missing_file(self, manager, mock_dsf, tmp_path):
        git_dir = tmp_path / ".git"
        git_dir.mkdir()
        sys_dir = tmp_path / "sys"
        sys_dir.mkdir()
        ...
        mock_dsf.get_file.return_value = None
    ...
    def test_diff_all_modified_file(self, manager, mock_dsf, tmp_path):
        git_dir = tmp_path / ".git"
        git_dir.mkdir()
        sys_dir = tmp_path / "sys"
        sys_dir.mkdir()
    ...
    def test_diff_all_skips_unmanaged_paths(self, manager, mock_dsf, tmp_path):
        git_dir = tmp_path / ".git"
        git_dir.mkdir()
    ...
    def test_diff_all_mixed_statuses(self, manager, mock_dsf, tmp_path):
        git_dir = tmp_path / ".git"
        git_dir.mkdir()
        sys_dir = tmp_path / "sys"
        sys_dir.mkdir()
        ...
        def get_file_side_effect(path):
            if path == "0:/sys/config.g":
                return "G28\n"
            return None

        mock_dsf.get_file.side_effect = get_file_side_effect
...
class TestDiffFile:
    ...
    def test_diff_file_missing_on_printer(self, manager, mock_dsf, tmp_path):
        sys_dir = tmp_path / "sys"
        sys_dir.mkdir()
        (sys_dir / "config.g").write_text("G28\n", encoding="utf-8")
        mock_dsf.get_file.return_value = None
    ...
    def test_diff_file_unchanged(self, manager, mock_dsf, tmp_path):
        sys_dir = tmp_path / "sys"
        sys_dir.mkdir()
        (sys_dir / "config.g").write_text("G28\n", encoding="utf-8")
        mock_dsf.get_file.return_value = "G28\n"
    ...
    def test_diff_file_modified(self, manager, mock_dsf, tmp_path):
        sys_dir = tmp_path / "sys"
        sys_dir.mkdir()
        (sys_dir / "config.g").write_text("G28\nnew_line\n", encoding="utf-8")
        mock_dsf.get_file.return_value = "G28\nold_line\n"
...
class TestApplyAll:
    ...
    def test_apply_all_writes_all_managed_files(self, manager, mock_dsf, tmp_path):
        git_dir = tmp_path / ".git"
        git_dir.mkdir()
        sys_dir = tmp_path / "sys"
        sys_dir.mkdir()
        ...
        assert mock_dsf.put_file.call_count == 2

    def test_apply_all_skips_unmanaged_files(self, manager, mock_dsf, tmp_path):
        git_dir = tmp_path / ".git"
        git_dir.mkdir()
        ...
        mock_dsf.put_file.assert_not_called()

    def test_apply_all_creates_backups(self, manager, mock_dsf, tmp_path):
        git_dir = tmp_path / ".git"
        git_dir.mkdir()
...
class TestApplyFile:
    ...
    def test_apply_file_success(self, manager, mock_dsf, tmp_path):
        sys_dir = tmp_path / "sys"
        sys_dir.mkdir()
        (sys_dir / "config.g").write_text("G28\nnew\n", encoding="utf-8")
    ...
    def test_apply_file_creates_backups(self, manager, mock_dsf, tmp_path):
        sys_dir = tmp_path / "sys"
        sys_dir.mkdir()
        (sys_dir / "config.g").write_text("G28\n", encoding="utf-8")
...
class TestApplyHunks:
    ...
    def test_apply_hunks_printer_file_not_found(self, manager, mock_dsf, tmp_path):
        sys_dir = tmp_path / "sys"
        sys_dir.mkdir()
        (sys_dir / "config.g").write_text("G28\n", encoding="utf-8")
        mock_dsf.get_file.return_value = None
    ...
    def test_apply_hunks_no_valid_hunks(self, manager, mock_dsf, tmp_path):
        sys_dir = tmp_path / "sys"
        sys_dir.mkdir()
        (sys_dir / "config.g").write_text("G28\nnew\n", encoding="utf-8")
        mock_dsf.get_file.return_value = "G28\nold\n"
    ...
    def test_apply_hunks_success(self, manager, mock_dsf, tmp_path):
        sys_dir = tmp_path / "sys"
        sys_dir.mkdir()
        (sys_dir / "config.g").write_text("G28\nnew_line\n", encoding="utf-8")
        mock_dsf.get_file.return_value = "G28\nold_line\n"
        ...
        written_content = mock_dsf.put_file.call_args[0][1]
        assert "new_line" in written_content

    def test_apply_hunks_partial_failure(self, manager, mock_dsf, tmp_path):
        ...
        mock_dsf.get_file.return_value = current_content
    ...
    def test_apply_hunks_creates_backups(self, manager, mock_dsf, tmp_path):
        sys_dir = tmp_path / "sys"
        sys_dir.mkdir()
        (sys_dir / "config.g").write_text("G28\nnew\n", encoding="utf-8")
        mock_dsf.get_file.return_value = "G28\nold\n"
...
class TestCreateBackup:
    ...
    def test_create_backup_copies_printer_files(self, manager, mock_dsf, tmp_path):
        git_dir = tmp_path / ".git"
        git_dir.mkdir()
        sys_dir = tmp_path / "sys"
        sys_dir.mkdir()
        ...
        mock_dsf.get_file.return_value = "printer content"
    ...
    def test_create_backup_skips_unreadable_files(self, manager, mock_dsf, tmp_path):
        git_dir = tmp_path / ".git"
        git_dir.mkdir()
...
class TestRestoreBackup:
    def test_restore_backup_writes_files(self, manager, mock_dsf):
        with (
            patch.object(manager, "_create_backup"),
    ...
    def test_restore_backup_creates_safety_backups(self, manager, mock_dsf):
        with (
            patch.object(manager, "_create_backup") as mock_backup,
    ...
    def test_restore_backup_skips_unmanaged_paths(self, manager, mock_dsf):
        with (
            patch.object(manager, "_create_backup"),
