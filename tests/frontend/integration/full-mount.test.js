/**
 * Integration test: Full component mount with real Vuetify.
 *
 * Mounts the entire MeltingplotConfig component tree using real Vuetify
 * components (no stubs). This verifies that:
 *  - The component tree renders without errors
 *  - Vuetify components resolve correctly (v-tabs, v-card, v-btn, etc.)
 *  - Parent-child communication works (props, events)
 *  - Template bindings are valid
 *
 * If this test passes, the plugin will render in a real DWC instance.
 */
...
// Vuetify must be installed on the base Vue, not localVue
// (Vuetify 2 requirement — it attaches to the global Vue prototype)
...
// Mock fetch with a router: different responses per URL pattern
...
describe('Full-mount integration', () => {
    ...
    describe('Component tree renders', () => {
        ...
        it('renders ConfigStatus with real Vuetify list items', async () => {
            ...
            // ConfigStatus renders v-list-items with actual firmware info
        ...
        it('disables Check for Updates button when no repo URL', () => {
            ...
            expect(btn).toBeTruthy();
            // Vuetify renders disabled as attribute
    ...
    describe('ConfigDiff renders no-changes state', () => {
        it('shows "No changes detected" when diffFiles empty', async () => {
            ...
            // Navigate to Changes tab (index 1)
            const tabs = wrapper.findAll('.v-tab');
